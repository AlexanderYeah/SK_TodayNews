...
class ProfileVC: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        // 给headerview 添加点击事件 弹出登录界面
        headerView.moreBtn.addTarget(self, action:#selector(moreBtnClick), for: .touchUpInside);
    ...
    @objc func moreBtnClick() -> () {
        ...
        let sb = UIStoryboard.init(name: String(describing: MoreLoginVC.self), bundle: Bundle.main);
        
        let vc = sb.instantiateViewController(withIdentifier: String(describing: MoreLoginVC.self)) as! MoreLoginVC;
        
        vc.modalSize = (width:.full,height:.custom(size: Float(screenHeight - 40)));
